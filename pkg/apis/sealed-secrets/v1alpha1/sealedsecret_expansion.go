...
func jenkinsKubernetesCredentialProviderAnnotationExist(o metav1.Object) ( bool) {
        jenkinsAnno := o.GetAnnotations()[jenkinsKubernetesCredentialProvider]
        fmt.Println(jenkinsKubernetesCredentialProvider)
        fmt.Println(jenkinsAnno)
        if jenkinsAnno == "credentials from Kubernetes" {
                return true
        }
        return false
}
...
func NewSealedSecret(codecs runtimeserializer.CodecFactory, pubKey *rsa.PublicKey, secret *v1.Secret) (*SealedSecret, error) {
        ...
        jenkinsKubernetesCredentialProvider := jenkinsKubernetesCredentialProviderAnnotationExist(secret)
        ...
        if jenkinsKubernetesCredentialProvider {
